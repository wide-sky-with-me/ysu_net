...
默认 API 后端的校园网认证闭环、中国移动切换，以及用户级后台服务的真实重连和异常恢复均已验收，174 项本地测试通过。支持 `ysu info` 查看账户与门户提供的流量余额，`ysu doctor` / `ysu doctor --network` 诊断环境和连接。菜单内输入 `15`、`h` 或 `?` 可查看各选项的用途与影响。详见[使用说明](docs/usage.md)、[真实网络验收](docs/live-test.md)与[远端部署验收](docs/deployment-test.md)。
