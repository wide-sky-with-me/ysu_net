...
直接运行 `ysu`：首次使用可先输入 **15**（也支持 **h** 或 **?**）查看“选项说明”，逐项了解用途、是否联网、是否改配置，以及是否影响当前连接或后台服务。说明页只显示帮助，按 Enter 返回菜单，输入 0 退出。

菜单 **13** 查看账户与剩余流量，**7** 做本地诊断，**14** 做联网诊断。特别注意：**4 会下线断网，9 只停守护保留连接，0 只退出菜单；10/11 只改自启，不改变当前服务运行状态。**
