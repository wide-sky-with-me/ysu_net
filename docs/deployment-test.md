...
首次后台部署验收代码提交为 `6e1733e`（此前基线为 `1860ed4`），该轮记录提交为 `ceb6cd0`。账户与诊断的后续增强另见本文增量验收部分。
...
账户展示和 doctor 增强后，在同一校内主机使用独立临时源码完成只读验收，不停止正式守护、不变更认证状态：

- `info` 返回 0，显示实际中国移动服务，普通输出不再包含 `userIndex`。
- 门户实际只返回业务明细、我的运营商、在线设备；剩余流量与余额缺失，明确显示“门户未提供”，不输出虚构额度。
- `info --raw` 返回 0，保留 `online`、`account_http_status`、`account` 结构。
- `doctor` 返回 0，本地诊断 7 项通过；`doctor --network` 返回 0，9 项通过，验证实际服务与配置一致。
- 菜单新增账户查询与联网诊断入口；本地回归增加至 138 项，覆盖两层账户结构、数值零、HTML/控制字符、缺失数据、服务异常和 linger 提示。

## 后续正式部署与菜单验收

`e7e2ead` 已随后更新到正式部署目录：安装前仅停止守护以保留连接，安装后恢复后台，账户配置逐字节保持不变且权限为 600。正式入口 `info` 与 `doctor --network` 均通过，中国移动保持在线。

`cbd4706` 增加菜单“选项说明”。远端 GitHub 拉取超时后，经已有 SSH 传输 Git 增量包并核对完整提交哈希；不是复制未提交源码。真实终端中进入说明页、检查用途与影响、退出菜单均通过，前后后台进程 PID 不变，服务仍为 active。该阶段本地回归为 141 项。

以上为对应提交的历史验收事实，不代表后续每次整理提交都重新执行了登录、下线或后台故障注入。
